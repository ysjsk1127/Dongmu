...
export function addExpense(data) {
  if (!data.category) return { success: false, error: '지출 항목을 선택해주세요.' };
...
export function deleteExpense(id) {
  ...
  if (filtered.length === list.length) return { success: false, error: '해당 지출을 찾을 수 없습니다.' };
...
export function getTotalExpense(clubId) {
  return getExpenses(clubId).reduce((sum, e) => sum + (e.amount || 0), 0);
}
